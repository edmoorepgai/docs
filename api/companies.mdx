...
## Add Company (Async Enrichment)

Submit a company for discovery and enrichment. Returns `202 Accepted` with a `public_operation_id` to poll via `GET /operations/{public_operation_id}`.

Provide at least one of `company_name`, `domain`, or `id`.
...
<RequestExample>
```bash cURL
curl -X POST "https://api.getpg.ai/public-api/v1/companies" \
  -H "x-api-key: your_api_key" \
  -H "Content-Type: application/json" \
  -d '{
